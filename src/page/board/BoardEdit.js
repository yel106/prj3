...
export function BoardEdit() {
  ...
  function handleRemoveFileSwitch(e) {
    if (e.target.checked) {
      ...
      updateBoard(draft => {
        draft.boardFiles = draft.boardFiles.filter(file => file.id !== e.target.value);
      });
    } else {
  ...
  return (
    ...
    <Container>
      ...
      <FormControl>
        ...
        {board.boardFiles !== null && board.boardFiles.map(file => <Box key={file.id}>
            <Image src={file.fileUrl} alt={file.fileName} w="100%"/>
          <Text>등록되어 있는 이미지를 삭제하고 다시 등록 하시겠습니까?</Text>
          <Switch
            value={file.id}
                    colorScheme="pink"
